...
(function (global, factory) {
    ...
    var SECTION_ABOUT = '.section-about';
    var SECTION_MAIN = '.section-main';
    var SECTION_MAIN_ACTIVE = SECTION_MAIN + ACTIVE_SEL;
    var SECTION = 'fp-section';
    var SECTION_SEL = '.' + SECTION;
    var SECTION_ABOUT_ACTIVE = '.section-about' + ACTIVE_SEL;
    var TOGGLE_COLOR = '.toggle-color'
    ...
    $.fn.fullpage = function (options) {
        ...
        options = $.extend({
            ...
            onLeave: null,
            afterRender: null,
            afterResize: function reBuild(resizing) {
        ...
        function moveSectionUp() {
            if ($(SECTION_ABOUT_ACTIVE).hasClass('active')) {

                if (!$(SECTION_ABOUT_ACTIVE).hasClass('section-dark') && checkpoint === 2) {
                    ...
                    var prev = $(SECTION_ABOUT_ACTIVE).prev(SECTION_ABOUT);
                    //looping to the bottom if there's no more sections above
                    if (!prev.length && (options.loopTop || options.continuousVertical)) {
                        prev = $(SECTION_ABOUT).last();
                    }
                    if (prev.length) {
                        scrollPage(prev, null, true);
                    }

                    var prev2 = $(SECTION_ACTIVE_SEL).prev(SECTION_SEL);
                    //looping to the bottom if there's no more sections above
                    if (!prev2.length && (options.loopTop || options.continuousVertical)) {
                    ...
                    $(SECTION_ABOUT_ACTIVE).removeClass('section-dark')
                    $('.header').toggleClass('header-white')
                    $('.scroll').toggleClass('white')
                    $('.header.small ul').css('opacity', '1')
                    $(SECTION_ABOUT_ACTIVE).find(TOGGLE_COLOR).removeClass('active')

                    isScrollScreen = false
                }
            }
            var prev = $(SECTION_MAIN_ACTIVE).prev(SECTION_SEL);
            //looping to the bottom if there's no more sections above
            if (!prev.length && (options.loopTop || options.continuousVertical)) {
                prev = $(SECTION_MAIN).last();
            }
            if (prev.length) {
                scrollPage(prev, null, true);
            }
        ...
        function moveSectionDown() {
            if ($(SECTION_ABOUT_ACTIVE).hasClass('active')) {
                ...
                if ($(SECTION_ABOUT_ACTIVE).hasClass('section-dark') && checkpoint === 1) {
                    ...
                    var next = $(SECTION_ABOUT_ACTIVE).next(SECTION_ABOUT);
                    //looping to the top if there's no more sections below
                    if (!next.length &&
                        (options.loopBottom || options.continuousVertical)) {
                        next = $(SECTION_ABOUT).first();
                    }
                    if (next.length) {
                    ...
                    var next2 = $(SECTION_ACTIVE_SEL).next(SECTION_SEL);
                    //looping to the top if there's no more sections below
                    if (!next2.length &&
                        (options.loopBottom || options.continuousVertical)) {
                    ...
                    //                    next.removeClass('section-dark')
                } else if (checkpoint === 0 || checkpoint === 2) {
                    setTimeout(function () {
                        checkpoint = 1
                    }, 200)
                    ...
                    $(SECTION_ABOUT_ACTIVE).addClass('section-dark')
                    $('.header').toggleClass('header-white')
                    $('.scroll').toggleClass('white')
                    $('.header.small ul').css('opacity', '0')
                    $(SECTION_ABOUT_ACTIVE).find(TOGGLE_COLOR).addClass('active')
                    isScrollScreen = false
                }
            }

            var next = $(SECTION_MAIN_ACTIVE).next(SECTION_MAIN);

            //                        looping to the top
            //                        if there 's no more sections below
            if (!next.length &&
                (options.loopBottom || options.continuousVertical)) {
                next = $(SECTION_MAIN).first();
            }
            if (next.length) {
                scrollPage(next, null, false);
            }
        ...
        /**
         * Styling vertical sections высота секций при загрузке
         */
...
//                            if ((start.X > end.X && start.X - end.X < 80) ||
//                                (end.X > start.X && end.X - start.X < 80)) {
//                                if (end.Y - start.Y > 80) scrollSection();
//                                else if (start.Y - end.Y > 80) scrollSection();
//                            }
//                        }
//
//                        var start = {};
...
//                document.body.addEventListener("touchend", touchEnd);
        ...
        function preventBouncing(event) {
            ...
            if (options.autoScrolling && isReallyTouch(e)) {
                ...
                //                    if ((start.X > end.X && start.X - end.X < 80) ||
                //                        (end.X > start.X && end.X - start.X < 80)) {
                //                        if (end.Y - start.Y > 80) baseSlider.goTo('prev');
                //                        else if (start.Y - end.Y > 80) baseSlider.goTo('next');
                //                    }
                //                }
                //
                //                var start = {};
...
//                                if ((start.X > end.X && start.X - end.X < 80) ||
//                                    (end.X > start.X && end.X - start.X < 80)) {
//                                    if (end.Y - start.Y > 80) console.log('down')
//                                    else if (start.Y - end.Y > 80) scrolling('down');
//                                }
//                            }
//
//                            var start = {};
...
//                                if ((start.X > end.X && start.X - end.X < 80) ||
//                                    (end.X > start.X && end.X - start.X < 80)) {
//                                    if (end.Y - start.Y > 80) scrolling('up');
//                                    else if (start.Y - end.Y > 80) console.log('down')
//                                }
//                            }
//
//                            var start = {};
...
//                    if ((start.X > end.X && start.X - end.X < 80) ||
//                        (end.X > start.X && end.X - start.X < 80)) {
//                        if (end.Y - start.Y > 80) moveSectionUp();
//                        else if (start.Y - end.Y > 80) moveSectionUp();
//                    }
//                }
//
//                var start = {};
